...
import com.haochang.spring.cloud.alibaba.highqps.model.*;
import com.haochang.spring.cloud.alibaba.highqps.mq.MQProducer;
import com.haochang.spring.cloud.alibaba.highqps.service.AccessLimitService;
...
//                redisDAO.putSeckill(seckill);
    ...
    private SeckillExecution handleSeckillAsync(long seckillId, long userPhone, String md5)
        ...
        if (redisUtil.sHasKey(boughtKey, String.valueOf(userPhone))) {
...
//            mqProducer.send(msgBody);
    ...
    public void handleInRedis(long seckillId, long userPhone) throws SeckillException {
        ...
        redisUtil.decr(inventoryKey, 1L);
        redisUtil.sSet(boughtKey, String.valueOf(userPhone));
    ...
    public int isGrab(long seckillId, long userPhone) {
        ...
        try {
            String boughtKey = RedisKeyPrefix.BOUGHT_USERS + seckillId;
            result = redisUtil.sHasKey(boughtKey, String.valueOf(userPhone)) ? 1 : 0;
        } catch (Exception ex) {
        ...
        if (result == 0) {
            if (!redisUtil.sHasKey(RedisKey.QUEUE_PRE_SECKILL, seckillId + "@" + userPhone)) {
                result =2;
            }
        }
        return result;
